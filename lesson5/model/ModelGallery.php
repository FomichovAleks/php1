...
class ModelGallery
{
    ...
    {
        $this->dir = $dir;
        $this->db = new DataBas('127.0.0.1', 'alex', '12345', 'alex');
    }
//Функция возвращает объект фотографии (ассоциативный массив)
    ...
    {
        $photos = scandir($this->dir); // Получаем список файлов из этой директории без сортировки
        return $photos;
    }
    ...
    public function Add($max_file_size, $valid_extensions)
    {
        ...
        else
        {
            // массив ошибок
            $error_values = array(
                 UPLOAD_ERR_INI_SIZE => 'Размер файла  разрешенного директивой upload_max_filesize в php.ini',
                 UPLOAD_ERR_FORM_SIZE => 'Размер файла превышает указанное значение',
